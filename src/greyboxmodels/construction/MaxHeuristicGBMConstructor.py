"""
This class searches for the best GBM by iteratively selecting the GBM with the largest computational load and then
replacing it with a BBM, followed by an evaluation of the new GBM.
...
class GreyBoxModelConstructor:
    ...
    def construct(self) -> Tuple[tuple, Dict]:
        """
        This constructor implements the corresponding heuristic.
        TODO FIX
        """
        ...
        # Get the original computational loads fo
        ...
        scenarios = self.gt_data.scenarios

        # Main Loop
        for k, plan in enumerate(tqdm.tqdm(gbm_plans, desc="Iterating repository")):
            # Get the plan with the largest computational load
            computational_loads = self.NIG_priors_l1Fi][k]


            # Compute performance for the current GBM
            scenarios = sim.simulate_plan(selected_plan, scenarios, self.repository, self.work_dir, parallel=True,
                                          pbar_offset=0)
